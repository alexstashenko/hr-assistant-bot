...
class HRAssistantBot:
    ...
    def add_message_to_history(self, user_id: int, role: str, content: str):
        ...
        # Ограничиваем историю последними 40 сообщениями (20 пар)
        # Это только для контекста Claude, не влияет на счетчик демо
...
### **Убрано:**
- Команда `/new` полностью удалена
- Пользователи НЕ могут сбрасывать свой лимит

### **Добавлено:**

1. **Персистентность данных** (`user_data.json`)
   - Счетчик сообщений сохраняется между перезапусками бота
   - Невозможно обойти лимит перезапуском

2. **Команда `/grant` для админа**
```
   /grant 123456789
```
   - Только админ может сбросить лимит пользователя
   - Даёт пользователю новые 20 сообщений

3. **Защита от обхода**
   - Даже `/start` не сбрасывает счетчик
   - Лимит строго 20 сообщений для обычных пользователей
   - Админ имеет 1000 сообщений

## Пример работы:
```
Пользователь /start
Бот: "ДЕМО-РЕЖИМ: У вас 20 сообщений"

[20 сообщений]

Пользователь "Еще вопрос"
Бот: "ДЕМО-РЕЖИМ ЗАВЕРШЕН. Обратитесь к @alexander_stashenko"
Админу: "Пользователь ID 123456 завершил ДЕМО"

[Перезапуск бота]

Пользователь "Привет"
Бот: "ДЕМО-РЕЖИМ ЗАВЕРШЕН" (данные сохранены!)

Админ /grant 123456
Бот админу: "Лимит сброшен! Пользователь получил новые 20 сообщений"

Пользователь "Спасибо!"
Бот: [нормальный ответ, счетчик 1/20]
