...
Ты — **Навигатор смыслов**, проводник через кризис среднего возраста для людей 45-60+. Помогаешь находить личные ответы через сократический диалог, не даёшь готовые решения.
...
class HRAssistantBot:
    ...
    async def notify_admin_demo_complete(self, user_id: int, username: str, first_name: str):
        ...
        try:
            ...
            message = (
                ...
                f"Пользователь исчерпал 20 бесплатных сообщений.\n\n"
    ...
    def add_message_to_history(self, user_id: int, role: str, content: str):
        ...
        # Ограничиваем историю последними 40 сообщениями (20 пар) для контекста Claude
        # Но полная история сохраняется в файл
        if len(self.conversations[user_id]) > 40:
            # Оставляем только последние 40 для контекста
    ...
    async def get_claude_response(self, user_id: int, user_message: str) -> str:
        ...
        try:
            ...
            # Получаем историю разговора (последние 40 сообщений для контекста)
    ...
    async def start_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        welcome_message = (
            f"Здравствуйте! Я помогаю исследовать важные вопросы о жизни и смыслах.\n\n"
    ...
    async def grant_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not context.args or len(context.args) != 1:
            await update.message.reply_text(
                ...
                "Эта команда сбрасывает счетчик сообщений пользователя до 20."
            )
            return
        ...
        try:
            ...
            await update.message.reply_text(
                ...
                f"Username: @{username}\n"
                f"ID: {target_user_id}\n\n"
                f"Пользователь получил новые 20 сообщений."
            )
