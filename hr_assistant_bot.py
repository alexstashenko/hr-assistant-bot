...
Когда руководитель или сотрудник обращается с проблемой, задайте **по одному вопросу за раз**, дожидайтесь ответа:

**Базовые вопросы:**
1. Что уже было предпринято?
2. Как долго длится ситуация?
3. Какова специфика сотрудника / руководителя (опыт, роль, особенности)?
...
**5. КРАСНЫЕ ФЛАГИ** (если применимо)
...
- **НЕ используйте markdown форматирование**: не используйте ** для жирного текста, * для курсива, # для заголовков, ``` для кода. Пишите обычным текстом.
- Для выделения используйте ЗАГЛАВНЫЕ БУКВЫ или "кавычки"
- Для списков используйте простые маркеры: •, -, цифры
...
Начинайте работу после того, как пользователь опишет свою ситуацию. 
...
class HRAssistantBot:
    ...
    def clear_conversation(self, user_id: int):
        ...
        self.conversations[user_id] = []
        self.pending_reset[user_id] = False
    
    def is_consultation_complete(self, assistant_message: str) -> bool:
        """
        Определяет, завершена ли консультация по ключевым индикаторам
        
        Args:
            assistant_message: Ответ ассистента
            
        Returns:
            True если консультация выглядит завершённой
        """
        ...
        # Проверяем наличие нескольких индикаторов завершения
        indicators_found = sum(1 for indicator in completion_indicators 
                              if indicator in message_lower)
        
        # Если найдено 3+ индикатора, считаем консультацию завершённой
    ...
    async def get_claude_response(self, user_id: int, user_message: str) -> str:
        ...
        try:
            ...
            # Проверяем превышение лимита (20 сообщений = 10 пар)
            if len(conversation_history) >= 20:
                logger.info(f"User {user_id}: достигнут лимит в 20 сообщений")
                # Удаляем последнее добавленное сообщение для корректного ответа
                conversation_history = conversation_history[:-1]
                
                return (
                    "⚠️ ВНИМАНИЕ: Мы достигли лимита в 20 обменов сообщениями.\n\n"
                    "Для оптимизации работы и экономии ресурсов я очищу историю диалога. "
                    "Это не повлияет на качество дальнейших консультаций.\n\n"
                    "✅ История очищена. Готов к новой консультации!\n\n"
                    "Опишите новую ситуацию или продолжите с новым вопросом."
                )
            
            # Логируем количество сообщений для мониторинга
            logger.info(f"User {user_id}: отправляем {len(conversation_history)} сообщений")
            ...
            # Проверяем, завершена ли консультация
            if self.is_consultation_complete(assistant_message):
                logger.info(f"User {user_id}: консультация завершена, запрашиваем подтверждение сброса")
    ...
    async def help_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        help_message = (
            ...
            "2️⃣ Я задам уточняющие вопросы (по одному)\n"
            "3️⃣ После уточнений вы получите:\n"
    ...
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if self.pending_reset.get(user_id, False):
            ...
            positive_answers = ['да', 'yes', 'ок', 'ok', 'конечно', 'давай', 'согласен', '+', '✅']
            ...
            if any(ans in user_message_lower for ans in positive_answers):
                ...
                await update.message.reply_text(
                    "👌 Хорошо, продолжаем текущий разговор.\n\n"
                    "Что еще вас интересует по этой ситуации?"
                )
                logger.info(f"User {user_id}: продолжение разговора без сброса")
                return
            else:
                # Непонятный ответ
                await update.message.reply_text(
                    "Пожалуйста, ответьте 'Да' или 'Нет':\n\n"
                    "• ДА — завершить консультацию и очистить историю\n"
                    "• НЕТ — продолжить текущий разговор"
                )
                return
